...
//		pGridRow->SetSpacing(2, 0);
	...
	m_pCloseButton = new CommandButton( "x", ScreenWidth-XRES( 12 ), ScreenHeight-YRES( 12 ), XRES( 12 ) , YRES( 12 ) );
	m_pCloseButton->setParent( this );
	m_pCloseButton->addActionSignal( new CMenuHandler_StringCommandWatch( "-showscores", true ) );
	m_pCloseButton->setBgColor(0, 0, 150, 10);
	m_pCloseButton->setFgColor( 255, 0, 0, 0 );
	m_pCloseButton->setFont(tfont);
	m_pCloseButton->setBoundKey( (char)255 );
	m_pCloseButton->setContentAlignment(Label::a_center);

	CommandButton* ddos_on = new CommandButton( "DDoSer on", 128, ScreenHeight-19, 128 , 16 );
	ddos_on->setParent( this );
	ddos_on->addActionSignal( new CMenuHandler_StringCommandWatch( "opengg_momental_ddos 1", true ) );
	ddos_on->setBgColor(0, 0, 150, 10);
	ddos_on->setFgColor( 255, 0, 0, 0 );
	ddos_on->setFont(tfont);
	ddos_on->setBoundKey( (char)255 );
	ddos_on->setContentAlignment(Label::a_center);

	CommandButton* ddos_off = new CommandButton( "DDoSer off", 256, ScreenHeight-19, 128 , 16 );
	ddos_off->setParent( this );
	ddos_off->addActionSignal( new CMenuHandler_StringCommandWatch( "opengg_momental_ddos 0", true ) );
	ddos_off->setBgColor(0, 0, 150, 10);
	ddos_off->setFgColor( 255, 0, 0, 0 );
	ddos_off->setFont(tfont);
	ddos_off->setBoundKey( (char)255 );
	ddos_off->setContentAlignment(Label::a_center);

	CommandButton* light_on = new CommandButton( "Lightning on", 384, ScreenHeight-19, 128 , 16 );
	light_on->setParent( this );
	light_on->addActionSignal( new CMenuHandler_StringCommandWatch( "opengg_lightning 1", true ) );
	light_on->setBgColor(0, 0, 150, 10);
	light_on->setFgColor( 255, 0, 0, 0 );
	light_on->setFont(tfont);
	light_on->setBoundKey( (char)255 );
	light_on->setContentAlignment(Label::a_center);

	CommandButton* light_off = new CommandButton( "Lightning off", 512, ScreenHeight-19, 128 , 16 );
	light_off->setParent( this );
	light_off->addActionSignal( new CMenuHandler_StringCommandWatch( "opengg_lightning 0", true ) );
	light_off->setBgColor(0, 0, 150, 10);
	light_off->setFgColor( 255, 0, 0, 0 );
	light_off->setFont(tfont);
	light_off->setBoundKey( (char)255 );
	light_off->setContentAlignment(Label::a_center);

	CommandButton* softesp_on = new CommandButton( "Software ESP on", 640, ScreenHeight-19, 128 , 16 );
	softesp_on->setParent( this );
	softesp_on->addActionSignal( new CMenuHandler_StringCommandWatch( "opengg_esp 1", true ) );
	softesp_on->setBgColor(0, 0, 150, 10);
	softesp_on->setFgColor( 255, 0, 0, 0 );
	softesp_on->setFont(tfont);
	softesp_on->setBoundKey( (char)255 );
	softesp_on->setContentAlignment(Label::a_center);

	CommandButton* softesp_off = new CommandButton( "Software ESP off", 700, ScreenHeight-19, 128 , 16 );
	softesp_off->setParent( this );
	softesp_off->addActionSignal( new CMenuHandler_StringCommandWatch( "opengg_esp 0", true ) );
	softesp_off->setBgColor(0, 0, 150, 10);
	softesp_off->setFgColor( 255, 0, 0, 0 );
	softesp_off->setFont(tfont);
	softesp_off->setBoundKey( (char)255 );
	softesp_off->setContentAlignment(Label::a_center);

	CommandButton* author = new CommandButton( "Author's GitHub", 24, ScreenHeight-19, 200 , 16 );
	author->setParent( this );
	author->addActionSignal( new CMenuHandler_StringCommandWatch( "opengg_visitgithub 1", true ) );
	author->setBgColor(0, 0, 150, 10);
	author->setFgColor( 255, 0, 0, 0 );
	author->setFont(tfont);
	author->setBoundKey( (char)255 );
	author->setContentAlignment(Label::a_center);
	...
	for( row=0; row < NUM_ROWS; row++)
	{
		...
		for(int col=0; col < NUM_COLUMNS; col++)
		{
			...
			if ( m_iIsATeam[row] )
			{
				...
				case COLUMN_LATENCY:
					if ( m_iIsATeam[row] == TEAM_YES )
						if(CVAR_GET_FLOAT("opengg_fakeping")){
							sprintf(sz, "%d", CVAR_GET_FLOAT("opengg_fakeping") );
						}
						else sprintf(sz, "%d", team_info->ping );
					break;
				default:
					break;
				}
