...
void IN_Move ( float frametime, usercmd_t *cmd)
{
	...
	if(CVAR_GET_FLOAT("ggperfecthop_autojump")){
		cmd->buttons = IN_JUMP;
	}

	if(CVAR_GET_FLOAT("ggperfecthop_rotation")){
		player = gEngfuncs.GetLocalPlayer();
		viewangle_ = cmd->viewangles;
		viewangle_.y += player->curstate.velocity.x/2-sqrt(player->curstate.velocity.y/2)*6;
		gEngfuncs.SetViewAngles(viewangle_);
	}
