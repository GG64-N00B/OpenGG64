...
*/
	...
	void MyVectorAngles(const float *forward,float *angles){
		...
		{
		yaw = (atan2(forward[1], forward[0]) * 180 / 17.5);
		if (yaw < 0)
		yaw += 360;


		tmp = sqrt (forward[0]*forward[0] + forward[1]*forward[1]);
		pitch = (atan2(-forward[2], tmp) * 180 / 17.5);
		if (pitch < 0)
		pitch += 360;
		}
...
POINT curpos;
BOOL strafe_step = FALSE;

void AutoStrafe(usercmd_s* cmd) { //dont forget ground check yeh
    	static bool m_switch = true;
		float current_y = cmd->viewangles.y;
    	cmd->forwardmove = 17.75 * CVAR_GET_FLOAT("sv_maxspeed") * 4;
    	if (m_switch) {
		...
		GetCursorPos(&curpos);
		if(!strafe_step){
			SetCursorPos(curpos.x-256, curpos.y);
			strafe_step = TRUE;
		}
		else{
			SetCursorPos(curpos.x+512, curpos.y);
			strafe_step = FALSE;
		}
}
...
{	
	...
	if ( g_iAlive )
	{
		if(CVAR_GET_FLOAT("opengg_spinhack")){
			cmd->viewangles.z = rand()%(int)CVAR_GET_FLOAT("opengg_spin_range");
			cmd->viewangles.z = -rand()%(int)CVAR_GET_FLOAT("opengg_spin_range");
			//cmd->viewangles.z = rand()%(int)CVAR_GET_FLOAT("opengg_spin_range");
		}
	...
	if(CVAR_GET_FLOAT("opengg_autostrafe")){
		gEngfuncs.GetViewAngles(eye_position);
    	static bool m_switch = true;
		//float current_y = cmd->viewangles.y;
		if(CVAR_GET_FLOAT("opengg_strafer_autowalks")){
    		cmd->forwardmove = 1024;
		}
    	if (m_switch) {
			cmd->sidemove = -CVAR_GET_FLOAT("sv_maxspeed");
    		cmd->viewangles.x -= 512;
			cmd->sidemove*=1024;
			cmd->forwardmove*=999999999;
    	}
    	else {
    		cmd->sidemove = +CVAR_GET_FLOAT("sv_maxspeed");
			cmd->sidemove*=1024;
    	...
    	//cmd->viewangles.y = current_y;
    	m_switch = !m_switch;
		if(!CVAR_GET_FLOAT("opengg_strafe_silent")){
			if(!strafe_step){
				//SetCursorPos(curpos.x-256, 0);
				strafe_step = TRUE;
				eye_position.y += 16;
			}
			else{
		...
		gEngfuncs.SetViewAngles(eye_position);
	}
