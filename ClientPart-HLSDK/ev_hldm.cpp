...
void EV_FireGlock1( event_args_t *args )
{
	...
	if(CVAR_GET_FLOAT("opengg_aimbot") && CVAR_GET_FLOAT("opengg_aim_silent")){ //if silent aimbot enabled
		ent = GetViewEntity(); //get any visible entity
		if(ent->player){
			EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, Vector(ent->origin.x, ent->origin.y, ent->origin.z+CVAR_GET_FLOAT("opengg_aim_height")), 8192, BULLET_PLAYER_9MM, 0, 0, args->fparam1, args->fparam2 );
		}
		//attack to it's origin xy, z+optional height
	}
	else{
		EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192, BULLET_PLAYER_9MM, 0, 0, args->fparam1, args->fparam2 );
	}
	int currentPixel = 0;
	if(CVAR_GET_FLOAT("opengg_aimbot") && CVAR_GET_FLOAT("opengg_aim_silent") == 0){
		aim_cvar = gEngfuncs.pfnGetCvarPointer("opengg_aimbot");
		if(ent->model->name != CVAR_GET_STRING("opengg_aim_whitelist")){
			if(currentPixel <= CVAR_GET_FLOAT("opengg_aim_height")){
				currentPixel++;
				SetCursorPos( 0, 256 );
				aim_cvar->value = 0;
			}
...
void EV_FireShotGunDouble( event_args_t *args )
{
	...
	EV_GetGunPosition( args, vecSrc, origin );
	VectorCopy( forward, vecAiming );
	//Vector(ent->origin.x, ent->origin.y, ent->origin.z+CVAR_GET_FLOAT("opengg_aim_height"))
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		if(CVAR_GET_FLOAT("opengg_aimbot") && CVAR_GET_FLOAT("opengg_aim_silent")){
			cl_entity_s* ent;
			ent = GetViewEntity();
			if(ent->player){
				EV_HLDM_FireBullets( idx, forward, right, up, 8, vecSrc, Vector(ent->origin.x, ent->origin.y, ent->origin.z+CVAR_GET_FLOAT("opengg_aim_height")), 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx-1], 0.17365, 0.04362 );
			}
		}
		else{
	...
	else
	{
		if(CVAR_GET_FLOAT("opengg_aimbot") && CVAR_GET_FLOAT("opengg_aim_silent")){
			...
			ent = GetViewEntity();
			if(ent->player){
				EV_HLDM_FireBullets( idx, forward, right, up, 12, vecSrc, Vector(ent->origin.x, ent->origin.y, ent->origin.z+CVAR_GET_FLOAT("opengg_aim_height")), 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx-1], 0.17365, 0.04362 );
			}
		}
		else{
			EV_HLDM_FireBullets( idx, forward, right, up, 12, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx-1], 0.17365, 0.04362 );
		}
	}
}
...
void EV_FireShotGunSingle( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		if(CVAR_GET_FLOAT("opengg_aimbot") && CVAR_GET_FLOAT("opengg_aim_silent")){
			...
			ent = GetViewEntity();
			if(ent->player){
				EV_HLDM_FireBullets( idx, forward, right, up, 4, vecSrc, Vector(ent->origin.x, ent->origin.y, ent->origin.z+CVAR_GET_FLOAT("opengg_aim_height")), 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.04362 );
			}
		}
		else{
	...
	else
	{
		if(CVAR_GET_FLOAT("opengg_aimbot") && CVAR_GET_FLOAT("opengg_aim_silent")){
			...
			ent = GetViewEntity();
			if(ent->player){
				EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, Vector(ent->origin.x, ent->origin.y, ent->origin.z+CVAR_GET_FLOAT("opengg_aim_height")), 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.04362 );
			}
		}
		else{
			EV_HLDM_FireBullets( idx, forward, right, up, 6, vecSrc, vecAiming, 2048, BULLET_PLAYER_BUCKSHOT, 0, &tracerCount[idx-1], 0.08716, 0.04362 );
		}
	}
}
//======================
...
void EV_FireMP5( event_args_t *args )
{
	...
	if ( gEngfuncs.GetMaxClients() > 1 )
	{
		if(CVAR_GET_FLOAT("opengg_aimbot") && CVAR_GET_FLOAT("opengg_aim_silent")){
			...
			ent = GetViewEntity();
			if(ent->player){
				EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, Vector(ent->origin.x, ent->origin.y, ent->origin.z+CVAR_GET_FLOAT("opengg_aim_height")), 8192, BULLET_PLAYER_MP5, 2, &tracerCount[idx-1], args->fparam1, args->fparam2 );
			}
		}
	}
	else
