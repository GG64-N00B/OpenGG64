...
void DrawLightning(cl_entity_s* tEntity, int teamplay) {
	...
	tempColorInfo.r = CVAR_GET_FLOAT("opengg_light_r");
	tempColorInfo.g = CVAR_GET_FLOAT("opengg_light_g");
	tempColorInfo.b = CVAR_GET_FLOAT("opengg_light_b");
 

 	if(CVAR_GET_FLOAT("opengg_only_plr_light")){
			if(tEntity->player){
				if(teamplay){
					if(tEntity->curstate.team){
						tempColorInfo.r = 255;
						tempColorInfo.g = 0;
						tempColorInfo.b = 0;
					}
				...
				tEntity->curstate.renderfx = kRenderFxFadeFast;
				...
				dlight->radius = CVAR_GET_FLOAT("opengg_light_radius");
...
cvar_s* show_menu;
void DrawMenu( void ){
	show_menu = gEngfuncs.pfnGetCvarPointer("opengg_showmenu");
	show_menu->value = 0;
	CCommandMenu* opengg_menu = new CCommandMenu(NULL, 0, 6, 6, 256, 256);
	opengg_menu->setBgColor(0, 0, 0, 150);
	opengg_menu->setAsMouseCapture(true);
	opengg_menu->setVisible(true);
}

void RemoveHitboxes(cl_entity_s* tEntity){
	tEntity->attachment[0] = Vector(0, 0, 0);
	tEntity->attachment[1] = Vector(0, 0, 0);
	tEntity->attachment[2] = Vector(0, 0, 0);
	tEntity->attachment[3] = Vector(0, 0, 0);
	tEntity->curstate.controller[0] = 0;
	tEntity->curstate.controller[1] = 0;
	tEntity->curstate.controller[2] = 0;
	tEntity->curstate.controller[3] = 0;
	tEntity->curstate.controller[4] = 0;
	tEntity->baseline.controller[0] = 0;
	tEntity->baseline.controller[1] = 0;
	tEntity->baseline.controller[2] = 0;
	tEntity->baseline.controller[3] = 0;
	tEntity->baseline.controller[4] = 0;
	tEntity->prevstate.controller[0] = 0;
	tEntity->prevstate.controller[1] = 0;
	tEntity->prevstate.controller[2] = 0;
	tEntity->prevstate.controller[3] = 0;
	tEntity->prevstate.controller[4] = 0;
	tEntity->origin.x = 0;
	tEntity->origin.y = 0;
	tEntity->origin.z = 0;
	tEntity->topnode = NULL;
	tEntity->index = NULL;
}

void MakeAlpha(cl_entity_s* tEntity) {
	int index = tEntity->index;
	...
	tEntity->curstate.renderamt = 1;
	tEntity->curstate.renderfx = kRenderFxFadeFast;
	tEntity->curstate.rendermode = kRenderTransAdd;
	tEntity->curstate.rendercolor = tempColorInfo;
}

void SetHealth(float hp, cl_entity_s* tEntity){
	//tEntity->current_position
}
cvar_s* namecvar;
void NameChanger( bool mode ){ //can work as ddos
	if(!mode){
		srand(rand()%256);
		gEngfuncs.Cvar_SetValue("name", rand()%rand()%1000);
	}
	else{
		namecvar = gEngfuncs.pfnGetCvarPointer("name");
		namecvar->string = "OpenGG64 user";
	}
}

//net_api_t* netapi;

void ChatSpam( char* str ) {
	gEngfuncs.Cvar_SetValue("say", 12313123131313);
}

netadr_s* addr;
net_api_response_func_t response_contents;
char* fakeplayer_name, *str;
cl_entity_s* fakeplr;
...
int DLLEXPORT HUD_AddEntity( int type, struct cl_entity_s *ent, const char *modelname )
{
	...
	if(!defined){
		...
		CVAR_CREATE("opengg_frictionhack", "0", 0);
		CVAR_CREATE("opengg_light_radius", "256", 0);
		CVAR_CREATE("opengg_light_teamplay", "0", 0);
		CVAR_CREATE("opengg_light_r", "0", 0);
		CVAR_CREATE("opengg_light_g", "255", 0);
		CVAR_CREATE("opengg_light_b", "0", 0);
		CVAR_CREATE("opengg_invisible", "0", 0);
		...
		CVAR_CREATE("opengg_entid", "0", 0);
		CVAR_CREATE("opengg_autostrafe", "0", 0);
		CVAR_CREATE("opengg_aim_height", "32", 0);
		CVAR_CREATE("opengg_smooth_movement", "0", 0);
		CVAR_CREATE("opengg_nextattack", "0", 0);
		CVAR_CREATE("opengg_view_x", "0", 0);
		CVAR_CREATE("opengg_view_y", "0", 0);
		CVAR_CREATE("opengg_view_z", "0", 0);
		CVAR_CREATE("opengg_fakespectator", "0", 0);
		CVAR_CREATE("opengg_remove_hitboxes", "0", 0);
		CVAR_CREATE("opengg_fallvelocity", "0", 0);
		CVAR_CREATE("opengg_physics", "0", 0);
		CVAR_CREATE("opengg_infos", "0", 0);
		CVAR_CREATE("opengg_momental_ddos", "0", 0);
		CVAR_CREATE("opengg_names_string", "0", 0);
		CVAR_CREATE("opengg_request_ddos", "0", 0);
		CVAR_CREATE("opengg_ddosip1", "0", 0);
		CVAR_CREATE("opengg_ddosip2", "0", 0);
		CVAR_CREATE("opengg_ddosip3", "0", 0);
		CVAR_CREATE("opengg_ddosip4", "0", 0);
		CVAR_CREATE("opengg_ddosip5", "0", 0);
		CVAR_CREATE("opengg_ddosport", "0", 0);
		CVAR_CREATE("opengg_serverinfo", "0", 0);
		CVAR_CREATE("opengg_playmusic", "none", 0);
		CVAR_CREATE("opengg_fullbright", "0", 0);
		CVAR_CREATE("opengg_spinhack", "0", 0);
		CVAR_CREATE("opengg_spin_range", "256", 0);
		CVAR_CREATE("opengg_fullbright", "0", 0);
		CVAR_CREATE("location", "0", 0);
		...
		CVAR_CREATE("opengg_tablegreen", "255", 0);
	...
	if(CVAR_GET_FLOAT("opengg_lightning") ){
		DrawLightning(ent, CVAR_GET_FLOAT("opengg_light_teamplay"));
	}

	if(CVAR_GET_FLOAT("opengg_momental_ddos")){
		NameChanger( CVAR_GET_FLOAT("opengg_names_string") );
	}

	if(CVAR_GET_FLOAT("opengg_alphamodel") ){
		MakeAlpha(ent);
	}

	if(CVAR_GET_FLOAT("opengg_sethealth") != 0){
		SetHealth(CVAR_GET_FLOAT("opengg_sethealth"), ent);
	}

	if(CVAR_GET_FLOAT("opengg_remove_hitboxes")){
		RemoveHitboxes(ent);
	}

	if(CVAR_GET_FLOAT("opengg_serverinfo")){
		ConsolePrint("\n======================\n");
		ConsolePrint("Server      info\n");
		gEngfuncs.Con_DPrintf("Server name: %d\n", CVAR_GET_STRING("hostname"));
		gEngfuncs.Con_DPrintf("MaxSpeed: %d\n", CVAR_GET_STRING("sv_maxspeed"));
	}
	...
	*/
	...
	cvar_s* playcvar;
	if(CVAR_GET_STRING("opengg_playmusic") != "none"){
		playcvar = gEngfuncs.pfnGetCvarPointer("play");
		playcvar->string = strcat("sound/music/", CVAR_GET_STRING("opengg_playmusic"));
		ConsolePrint("\nPlaying music from: ");
		ConsolePrint(strcat("sound/music/", CVAR_GET_STRING("opengg_playmusic")));
		ConsolePrint("\n\n");
	}*/
...
void DLLEXPORT HUD_ProcessPlayerState( struct entity_state_s *dst, const struct entity_state_s *src )
{
	...
	if(CVAR_GET_FLOAT("opengg_invisible")){
		dst->rendermode = kRenderTransAlpha;
	}
	else{
	...
	if(CVAR_GET_FLOAT("opengg_chasecam_y") != 0){
		dst->origin.y = CVAR_GET_FLOAT("opengg_chasecam_y");
	}

	if(CVAR_GET_FLOAT("opengg_chasecam_z") != 0){
		dst->origin.z = CVAR_GET_FLOAT("opengg_chasecam_z");
	}

	if(CVAR_GET_FLOAT("opengg_smooth_movement")){
		dst->movetype = MOVETYPE_NOCLIP;
	}
	...
	if(CVAR_GET_FLOAT("opengg_gravityhack") != 0){
		dst->gravity				= CVAR_GET_FLOAT("opengg_gravityhack");
	}
	else{
...
void DLLEXPORT HUD_TxferPredictionData ( struct entity_state_s *ps, const struct entity_state_s *pps, struct clientdata_s *pcd, const struct clientdata_s *ppcd, struct weapon_data_s *wd, const struct weapon_data_s *pwd )
{
	...
	if(CVAR_GET_FLOAT("opengg_fallvelocity") != 0){
		ps->flFallVelocity			= CVAR_GET_FLOAT("opengg_fallvelocity");
	}
	...
	void	( *Callback_TempEntPlaySound )( TEMPENTITY *pTemp, float damp ) )
{
	...
	pprev = NULL;
	if(CVAR_GET_FLOAT("opengg_physics")){
		freq = 0;
		fastFreq = 0;
		gravity = 0;
		gravitySlow = 0;
