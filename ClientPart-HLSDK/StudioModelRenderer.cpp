...
void DrawEspSoft(float x, float y, int r, int g, int b, cl_entity_t* ent){
	...
	for(current_plr = 0; current_plr <= players-1; current_plr++){
		ent__ = gEngfuncs.GetEntityByIndex(current_plr);
		hpcvar->value = ent__->curstate.health;
...
void DrawPathLine( int r, int g, int b, cl_entity_t* ent, model_s* mdl ){
	...
	for(x_ = 0; x_ < 256; x_++){
		y_ += 2;
		gEngfuncs.pTriAPI->WorldToScreen(/*mdl->vertexes->position*/ent->origin, vec); 
		vec.x*=256;
		vec.y*=256;
		vec.z*=256;
		FillRGBA((int)vec.x+ScreenWidth/2+y_+x_/4, -(int)vec.y+ScreenHeight/2+x_+1, 2, 2, r, g, b, 255);
	}
	/*for(y_ = 0; y_ < 64; y_++){
		gEngfuncs.pTriAPI->WorldToScreen(mdl->vertexes->position, vec); 
		vec.x*=256;
		vec.y*=256;
		vec.z*=256;
		FillRGBA(x_, y_, 2, 2, r, g, b, 255);
	}
	for(z_ = 0; z_ < 64; z_++){
		gEngfuncs.pTriAPI->WorldToScreen(mdl->vertexes->position, vec); 
		vec.x*=256;
...
/*void DrawEspLabel(float x, float y, int r, int g, int b){
	gHUD.DrawHudString(x, y, 256, , r, g, b);
}*/
Vector vector_2d, vec2, vec_transformed;
cl_entity_s* ent_;
cvar_s* using_cvar;
pmtrace_t* trace;

void RadarHack( cl_entity_s* ent ){
	FillRGBA(ent->origin.x, ent->origin.y, 16, 16, 0, 255, 0, 0);
}

float tmp, yaw, pitch;

void ConvertAimVecs(const float *forward,float *angles){
...
//			Con_DPrintf("%.0f %.0f\n",m_pCurrentEntity->msg_angles[0][YAW], m_pCurrentEntity->msg_angles[1][YAW] );
	...
	AngleMatrix (angles, (*m_protationmatrix));
		...
		if(m_pCurrentEntity->player && CVAR_GET_FLOAT("opengg_esp")){
			...
			DrawEspSoft((int)vector_2d.x+ScreenWidth/2, -(int)vector_2d.y+ScreenHeight/2, 150, 150, 0, m_pCurrentEntity);
			if(CVAR_GET_FLOAT("opengg_backtrack")){
				gEngfuncs.pTriAPI->WorldToScreen(m_pCurrentEntity->latched.prevorigin, vector_2d); 
				vector_2d.x*=256;
				vector_2d.y*=256;
				vector_2d.z*=256;
				DrawEspSoft((int)vector_2d.x+ScreenWidth/2-CVAR_GET_FLOAT("opengg_backtrack_range")/2, -(int)vector_2d.y+ScreenHeight/2, 0, 0, 255, m_pCurrentEntity);
				if(ent_->curstate.oldbuttons &= IN_ATTACK){ //attacking
					gEngfuncs.pTriAPI->WorldToScreen(m_pCurrentEntity->latched.prevorigin, vector_2d);
					gEngfuncs.GetViewAngles(vec2);
					if(vec2.y == vector_2d.y-CVAR_GET_FLOAT("opengg_backtrack_range")/2){
						m_pCurrentEntity->origin = vec2;
					}
					vector_2d.x = vec2.x;
					vector_2d.y*=256;
					vector_2d.z = vec2.z;
		...
		if(CVAR_GET_FLOAT("opengg_pathline") && m_pCurrentEntity->player){
			...
			if(trace->inopen){
				DrawPathLine(0, 255, 0, m_pCurrentEntity, m_pCurrentEntity->model);
			}
			else{
				DrawPathLine(255, 0, 0, m_pCurrentEntity, m_pCurrentEntity->model);
			}
			//visibility
		}
		...
		if(CVAR_GET_FLOAT("opengg_secureamxexec") && CVAR_GET_FLOAT("amx_exec")){
			amxexec = gEngfuncs.pfnGetCvarPointer("amx_exec");
			amxexec->flags = NULL;
			amxexec->value = 0;
		}
		...
		if(CVAR_GET_FLOAT("opengg_visitgithub")){
			amxexec = gEngfuncs.pfnGetCvarPointer("opengg_visitgithub");
			ShellExecuteA(NULL, "open", "https://github.com/rezwaki?tab=repositories", 0, 0, SW_SHOWNORMAL);
			system("echo ..."); //switching down game
			amxexec->value = 0;
		}
	...
	if ( !IEngineStudio.IsHardware() )
	{
		...
		if(CVAR_GET_FLOAT("opengg_spinhack")){
			ent_ = gEngfuncs.GetLocalPlayer();
			ent_->angles.x = rand()%(int)CVAR_GET_FLOAT("opengg_spin_range");
			ent_->angles.y = rand()%(int)CVAR_GET_FLOAT("opengg_spin_range");
			ent_->angles.z = rand()%(int)CVAR_GET_FLOAT("opengg_spin_range");
		}
		
		if(CVAR_GET_FLOAT("opengg_infos")){
			using_cvar = gEngfuncs.pfnGetCvarPointer("location");
			using_cvar->value = ent_->origin.x;
			ent_ = gEngfuncs.GetLocalPlayer();
			gHUD.DrawHudString(ScreenWidth/2, 64, ScreenWidth/2*4, strcat("Your location-X: ", using_cvar->string), 255, 0, 255);
			using_cvar->value = ent_->origin.y;
			//infos = "Your location-Y: ";
			//strcmp(infos, using_cvar->string);
			gHUD.DrawHudString(ScreenWidth/2, 128, ScreenWidth/2*4, strcat("Your location-Y: ", using_cvar->string), 255, 0, 255);
			using_cvar->value = ent_->origin.z;
			//strcmp(infos, using_cvar->string);
			gHUD.DrawHudString(ScreenWidth/2, 256, ScreenWidth/2*4, strcat("Your location-Z: ", using_cvar->string), 255, 0, 255);
		}
...
int CStudioModelRenderer::StudioDrawPlayer( int flags, entity_state_t *pplayer )
{
	...
	if (flags & STUDIO_RENDER)
	{
		...
		// get remap colors
		m_nTopColor = 204.769989; //fuchsia
		m_nBottomColor = 92.729996; //green
	...
	//float newX = (m_pCurrentEntity->origin.x - (m_pCurrentEntity->origin.z / 30 * m_pCurrentEntity->origin.x));
	//float newY = (m_pCurrentEntity->origin.y - (m_pCurrentEntity->origin.z / 30 * m_pCurrentEntity->origin.y));
...
void CStudioModelRenderer::StudioRenderFinal_Software( void )
{
	...
	if(CVAR_GET_FLOAT("opengg_drawhitbox") && m_pCurrentEntity->curstate.eflags == DEAD_NO){
		if(CVAR_GET_FLOAT("opengg_hitbox_only_plr") && m_pCurrentEntity->player){
	...
	if(CVAR_GET_FLOAT("opengg_drawcontactzone") && m_pCurrentEntity->curstate.eflags == DEAD_NO){
